...
export function UserPost({ userId }: { userId: string }) {
  // Filter post for this user
  const UserPost = post.filter((t) => t.userId === userId)

  if (UserPost.length === 0) {
    return <div className="text-center py-8 text-muted-foreground">No post found for this user.</div>
  }

  return (
    <div className="space-y-4">
      {UserPost.map((transaction) => (
