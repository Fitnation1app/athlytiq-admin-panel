...
async def get_user_history(id: str):
    try:
        ...
        # 2. Post Comments
        post_comments = supabase.table("post_comments").select("post_id, created_at").eq("user_id", id).execute().data or []
        ...
        # 3. Comments table
        comments = supabase.table("comments").select("post_id, created_at").eq("user_id", id).execute().data or []
